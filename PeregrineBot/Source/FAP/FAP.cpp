...
const auto score = [](const FastAPproximation::FAPUnit& fu) {
	if (fu.health && fu.maxHealth)
		return ((fu.score * fu.health) / (fu.maxHealth * 2)) + (fu.unitType == BWAPI::UnitTypes::Terran_Bunker) * BWAPI::UnitTypes::Terran_Marine.destroyScore() * 4;
	return 0;
};
...
FastAPproximation::FAPUnit::FAPUnit(BWAPI::Unit const u)
    : FAPUnit(EnemyData(u))
{
}
...
FastAPproximation::FAPUnit::FAPUnit(EnemyData ed)
    ...
    , groundCooldown(ed.lastType.groundWeapon().damageFactor() && ed.lastType.maxGroundHits()
                         ? ed.lastPlayer->weaponDamageCooldown(ed.lastType) / (ed.lastType.groundWeapon().damageFactor() * ed.lastType.maxGroundHits())
                         : 0)
    ...
    , airCooldown(ed.lastType.airWeapon().damageFactor() && ed.lastType.maxAirHits()
                      ? ed.lastType.airWeapon().damageCooldown() / (ed.lastType.airWeapon().damageFactor() * ed.lastType.maxAirHits())
                      : 0)
    ...
    , score(ed.lastType.destroyScore())
{
	...
	case BWAPI::UnitTypes::Protoss_Carrier:
		...
		if (ed.u && ed.u->isVisible()) {
			...
			if (ed.lastPlayer) {
				groundCooldown = static_cast<int>(round(37.0f / (ed.lastPlayer->getUpgradeLevel(BWAPI::UpgradeTypes::Carrier_Capacity) ? 8 : 4)));
			} else {
