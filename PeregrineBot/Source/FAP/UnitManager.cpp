...
//std::array<int, Neolib_fix::pow(256 * 4, 2)> DeathMatrixGround, DeathMatrixAir;

namespace Neolib {

EnemyData::EnemyData() = default;

EnemyData::EnemyData(BWAPI::Unit unit)
    : u(unit)
{
	if (unit)
		initFromUnit();
...
EnemyData::EnemyData(BWAPI::UnitType ut)
    : u(nullptr)
    , lastType(ut)
    , lastPlayer(BWAPI::Broodwar->self())
    , lastPosition({})
...
int EnemyData::expectedHealth() const
{
	if (lastType.getRace() == BWAPI::Races::Zerg)
		return std::min(
		    ((BWAPI::Broodwar->getFrameCount() - frameLastDetected) * ZERGREGEN) / 256 + lastHealth,
...
int EnemyData::expectedShields() const
{
	if (lastType.getRace() == BWAPI::Races::Protoss)
		return std::min(((BWAPI::Broodwar->getFrameCount() - frameLastDetected) * PROTOSSSHEILDREGEN) / 256 + lastShields,
		                lastType.maxShields());
	return lastShields;
}
...
//	if (!death || ed->expectedHealth() + ed->expectedShields() + ed->u->getDefenseMatrixPoints() == 0)
//		return 0;
...
//	}
//
//	if (isOwn(unit) && unit->getType() == BWAPI::UnitTypes::Terran_Command_Center && unitManager.countUnit(BWAPI::UnitTypes::Terran_Command_Center, BWAPI::Filter::Exists) > 1) {
//		buildingQueue.doBuild(BWAPI::UnitTypes::Terran_Bunker,
//		                      (BWAPI::TilePosition)unit->getPosition());
//	}
//}
